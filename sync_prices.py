...
def synchroniser_toutes_les_series():
    ...
    CONFIG_SERIES = [
        ...
        ("Starter Deck 7 Big Mom Pirates", "22930", "ST07"), ("Starter Deck 8 Monkey.D.Luffy", "22956", "ST08"),
        ("Starter Deck 9 Yamato", "22957", "ST09"), ("Ultra Deck The Three Captains", "23243", "ST10"),
        ("Starter Deck 11 Uta", "23250", "ST11"), ("Starter Deck 12 Zoro and Sanji", "23348", "ST12"),
        ("Ultra Deck The Three Brothers", "23349", "ST13"), ("Starter Deck 14 3D2Y", "23489", "ST14"),
        ("Starter Deck 15 RED Edward.Newgate", "23490", "ST15"), ("Starter Deck 16 GREEN Uta", "23491", "ST16"),
        ("Starter Deck 17 BLUE Donquixote Doflamingo", "23492", "ST17"), ("Starter Deck 18 PURPLE Monkey.D.Luffy", "23493", "ST18"),
        ("Starter Deck 19 BLACK Smoker", "23494", "ST19"), ("Starter Deck 20 YELLOW Charlotte Katakuri", "23495", "ST20"),
        ("Starter Deck EX Gear 5", "23991", "ST21"), ("Starter Deck 22 Ace & Newgate", "24304", "ST22"),
        ("Starter Deck 23 RED Shanks", "24282", "ST23"), ("Starter Deck 24 GREEN Jewelry Bonney", "24283", "ST24"),
    ...
    MAPPING_SERIES_DECK = {
        "StarterDeck1StrawHatCrewProductsAndPrices.csv": "ST01", "StarterDeck2WorstGenerationProductsAndPrices.csv": "ST02",
        "StarterDeck3TheSevenWarlordsofTheSeaProductsAndPrices.csv": "ST03", "StarterDeck4AnimalKingdomPiratesProductsAndPrices.csv": "ST04",
        "StarterDeck5FilmEditionProductsAndPrices.csv": "ST05", "StarterDeck6AbsoluteJusticeProductsAndPrices.csv": "ST06",
        "StarterDeck7BigMomPiratesProductsAndPrices.csv": "ST07", "StarterDeck8Monkey.D.LuffyProductsAndPrices.csv": "ST08",
        "StarterDeck9YamatoProductsAndPrices.csv": "ST09", "UltraDeckTheThreeCaptainsProductsAndPrices.csv": "ST10",
        "StarterDeck11UtaProductsAndPrices.csv": "ST11", "StarterDeck12ZoroandSanjiProductsAndPrices.csv": "ST12",
        "UltraDeckTheThreeBrothersProductsAndPrices.csv": "ST13", "StarterDeck143D2YProductsAndPrices.csv": "ST14",
        "StarterDeck15REDEdward.NewgateProductsAndPrices.csv": "ST15", "StarterDeck16GREENUtaProductsAndPrices.csv": "ST16",
        "StarterDeck17BLUEDonquixoteDoflamingoProductsAndPrices.csv": "ST17", "StarterDeck18PURPLEMonkey.D.LuffyProductsAndPrices.csv": "ST18",
        "StarterDeck19BLACKSmokerProductsAndPrices.csv": "ST19", "StarterDeck20YELLOWCharlotteKatakuriProductsAndPrices.csv": "ST20",
        "StarterDeckEXGear5ProductsAndPrices.csv": "ST21", "StarterDeck22Ace&NewgateProductsAndPrices.csv": "ST22",
        "StarterDeck23REDShanksProductsAndPrices.csv": "ST23", "StarterDeck24GREENJewelryBonneyProductsAndPrices.csv": "ST24",
    ...
    EXCLUSIONS_PAR_SERIE = {"D": ["D-OP01-001", "D-OP01-005", "D-OP01-011", "D-OP01-016", "D-OP01-021", "D-OP02-015", "D-P-001", "D-P-022", "D-P-080", "D-ST01-005", "D-ST01-006", "D-ST01-011", "D-ST01-014", "D-ST01-015", "D-ST01-017"]}
        ...
        with open(chemin_csv, mode="r", encoding="utf-8", newline="") as f:
            for ligne in csv.DictReader(f):
                ...
                if not num_brut and code_serie == "P":
                    ...
                    if match: num_brut = match.group(0)

                mots_exclus = ["Booster", "Box", "Case", "Pack", "Deck"] if code_serie not in ["P", "LD", "D"] else (["Booster Box", "Booster Case", "Display Box", "Sealed Case"] if code_serie in ["P", "LD"] else ["Promo"])

                if not num_brut or any(x.lower() in nom_produit.lower() for x in mots_exclus): continue

                num_normalise = normaliser_numero_carte(num_brut)
                serie_exacte = MAPPING_SERIES_DECK.get(nom_fichier_csv, code_serie)

                if code_serie.startswith("PRB") and not num_normalise.startswith(code_serie): num_normalise = f"{code_serie}-{num_normalise}"
                elif serie_exacte.startswith("ST") and not num_normalise.startswith(serie_exacte): num_normalise = f"{serie_exacte}-{num_normalise}"
                elif serie_exacte == "P" and not num_normalise.startswith("P-"): num_normalise = f"P-{num_normalise}"
                elif serie_exacte == "LD" and not num_normalise.startswith("LD-"): num_normalise = f"LD-{num_normalise}"
                elif serie_exacte == "D" and not num_normalise.startswith("D-"): num_normalise = f"D-{num_normalise}"

                if code_serie in EXCLUSIONS_PAR_SERIE and num_normalise in EXCLUSIONS_PAR_SERIE[code_serie]: continue
                ...
                if is_wanted or is_sp_or_tr:
                    ...
                    if not num_normalise.startswith(serie_exacte): num_normalise = f"{serie_exacte}-{num_normalise}"
                elif code_serie in SERIES_PROMO_AUTOMATIQUE: nom_propre = nom_produit
                else: nom_propre = nom_produit.split("(")[0].strip()
                ...
                except: m_price = None; mid_price = None
                ...
                if prix_brut and prix_brut > 0:
                    ...
                    disponible = True
                    if ancien_prix > 0:
                        if prix_final > ancien_prix: pourcentage = round(((prix_final - ancien_prix) / ancien_prix) * 100, 1); tendance = "hausse"
                        elif prix_final < ancien_prix: pourcentage = round(((prix_final - ancien_prix) / ancien_prix) * 100, 1); tendance = "baisse"
                        else: pourcentage = 0; tendance = "stable"
                    else: pourcentage = 0.0; tendance = "stable"
                else:
    ...
    with open(chemin_json, "w", encoding="utf-8") as f: json.dump(catalogue_global, f, indent=4, ensure_ascii=False)
